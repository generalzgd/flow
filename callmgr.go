...
* @func() 超时需要处理的方法
*/
func (p *WorkFlow) Call(id uint32, timeout time.Duration, prevFun func(), doneFun func(interface{}), timeoutFun func()) {
	prevFun()

	pip := p.newPip(id)
	defer func() {
		if err := recover(); err != nil {
			beego.Error("WorkFlow.Call panic, askid:", id, "err:", err, "stack:", string(debug.Stack()))
		}
		p.delPip(id)
	}()
...
func (p *WorkFlow) OnAnsyBack(id uint32, data interface{}) (succ bool) {
	defer func() {
		if err := recover(); err != nil {
			succ = false
			beego.Error("WorkFlow.OnAnsyBack panic, askid:", id, "err:", err, "stack:", string(debug.Stack()))
		}
	}()
